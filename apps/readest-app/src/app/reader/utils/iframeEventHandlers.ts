...
export const addLongPressListeners = (bookKey: string, doc: Document) => {
  ...
  // ── Text long-press → audiobook seek (selectionchange-based) ──────────────
  // On iOS, a timer-based long-press fires reliably for images but the browser
  // sends touchcancel when it takes over native text selection (~400 ms),
  // killing our timer. Instead we watch for selectionchange: when the browser's
  // own long-press gesture selects a word, we post the selected text.
  // The parent hook (useLongPressEvent) dispatches 'tts-audiobook-seek' only
  // when an audiobook is playing, so this is a no-op in normal reading mode.
  const LONG_PRESS_MIN_MS = 300; // selection must appear this long after touchstart
  const LONG_PRESS_MOVE_PX = 12; // cancel if finger moved more than this

  let lpTouchStart: { time: number; x: number; y: number } | null = null;

  const onLpTouchStart = (ev: TouchEvent) => {
    const t = ev.touches[0];
    if (!t) return;
    lpTouchStart = { time: Date.now(), x: t.clientX, y: t.clientY };
  };

  const onLpTouchMove = (ev: TouchEvent) => {
    if (!lpTouchStart) return;
    const t = ev.touches[0];
    if (!t) return;
    const dx = t.clientX - lpTouchStart.x;
    const dy = t.clientY - lpTouchStart.y;
    if (Math.sqrt(dx * dx + dy * dy) > LONG_PRESS_MOVE_PX) {
      lpTouchStart = null; // finger moved — not a long press
    }
  };
  ...
  const onSelectionChange = () => {
    if (!lpTouchStart) return;
    const elapsed = Date.now() - lpTouchStart.time;
    if (elapsed < LONG_PRESS_MIN_MS) return; // too quick — probably a tap
  ...
  doc.addEventListener('touchstart', onLpTouchStart, { passive: true });
  doc.addEventListener('touchmove', onLpTouchMove, { passive: true });
  doc.addEventListener('touchend', onLpTouchEnd);
  doc.addEventListener('touchcancel', onLpTouchEnd);
  doc.addEventListener('selectionchange', onSelectionChange);

  return () => {
    observer.disconnect();
    pressTimers.forEach((timer) => clearTimeout(timer));
    doc.removeEventListener('touchstart', onLpTouchStart);
    doc.removeEventListener('touchmove', onLpTouchMove);
    doc.removeEventListener('touchend', onLpTouchEnd);
    doc.removeEventListener('touchcancel', onLpTouchEnd);
